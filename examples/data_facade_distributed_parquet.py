...
def _seed_parquet_files(data_dir: Path) -> None:
    pd.DataFrame(
        {
...
def _load_and_join_parquet(project_root: Path, data_dir: Path) -> dict[str, object]:
    with LocalCluster(
        n_workers=2,
...
def run_example() -> dict[str, object]:
    with TemporaryDirectory() as tmp_dir:
        ...
        _seed_parquet_files(data_dir)

        result = _load_and_join_parquet(project_root, data_dir)
