...
def _seed_customers_and_orders(db_path: Path) -> None:
...
def _run_pandas_series_semi_join(config: SqlDatabaseConfig) -> pd.DataFrame:
    ...
    with DataHelper(config, table="orders") as helper:
        ...
        return ddf.compute().sort_values("id").reset_index(drop=True)


def _run_lazy_dask_series_semi_join(config: SqlDatabaseConfig) -> pd.DataFrame:
    ...
    with DataHelper(config, table="customers") as customer_helper:
        with DataHelper(config, table="orders") as order_helper:
            ...
            return orders_ddf.compute().sort_values("id").reset_index(drop=True)
...
def run_example() -> dict[str, object]:
    with TemporaryDirectory() as tmp_dir:
        ...
        _seed_customers_and_orders(db_path)
        ...
        pandas_result = _run_pandas_series_semi_join(config)
        lazy_result = _run_lazy_dask_series_semi_join(config)
