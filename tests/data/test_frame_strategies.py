...
def test_get_frame_strategy_returns_correct_types():
...
def test_get_frame_strategy_raises_on_unknown():
...
class TestDaskFrameStrategy:
    ...
    def test_normalize_returns_dask(self, simple_pd):
        result = self.s.normalize(simple_pd)
        assert isinstance(result, dd.DataFrame)

    def test_normalize_dask_passthrough(self, simple_dask):
        result = self.s.normalize(simple_dask)
        assert result is simple_dask

    def test_has_any_rows_true(self, simple_dask):
        assert self.s.has_any_rows(simple_dask) is True

    def test_concat(self, simple_dask):
        result = self.s.concat([simple_dask, simple_dask])
        assert isinstance(result, dd.DataFrame)
        assert len(result.compute()) == 6

    def test_rename_columns(self, simple_dask):
        result = self.s.rename_columns(simple_dask, {"a": "alpha"})
        assert "alpha" in result.columns

    def test_select_columns(self, simple_dask):
        result = self.s.select_columns(simple_dask, ["a"])
        assert list(result.columns) == ["a"]

    def test_apply_column_names(self, simple_dask):
        result = self.s.apply_column_names(simple_dask, ["col1", "col2"])
        assert list(result.columns) == ["col1", "col2"]

    def test_persist_returns_dask(self, simple_dask):
...
class TestPolarsFrameStrategy:
    ...
    def test_normalize_returns_polars(self, simple_polars):
        result = self.s.normalize(simple_polars)
    ...
    def test_has_any_rows_true(self, simple_polars):
    ...
    def test_concat(self, simple_polars):
        result = self.s.concat([simple_polars, simple_polars])
        assert result.height == 6

    def test_rename_columns(self, simple_polars):
        result = self.s.rename_columns(simple_polars, {"a": "alpha"})
        assert "alpha" in result.columns

    def test_select_columns(self, simple_polars):
        result = self.s.select_columns(simple_polars, ["a"])
        assert result.columns == ["a"]

    def test_apply_column_names(self, simple_polars):
        result = self.s.apply_column_names(simple_polars, ["col1", "col2"])
        assert result.columns == ["col1", "col2"]

    def test_apply_options_sort(self, simple_polars, no_params):
        opts = DataFrameOptions(sort_field="a")
    ...
    def test_apply_options_raises_on_datetime_index(self, simple_polars):
        opts = DataFrameOptions()
