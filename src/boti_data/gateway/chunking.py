...
class ChunkFanoutSettings:
    """Result-combination/concurrency/logging knobs for ChunkedLoadExecutor.load()."""
...
class InChunkPlanner:
    ...
    def _auto_controls(
        ...
        *,
        ...
        return self._non_eager_controls(hint)

    @staticmethod
    def _non_eager_controls(
...
class ChunkedLoadExecutor:
    ...
    # Not a copy-pasted twin: find_chunk_target()/iter_chunk_option_sets()/
    # _combine_results()/has_any_rows() are already shared static/classmethods;
    # the remaining difference is a genuine concurrency-model divergence
    ...
    def load(
        ...
        *,
        ...
        results = cls._execute_sync_chunks(execute_fn, chunk_opts_list, settings.max_concurrency)
        return cls._combine_results(results, return_type=settings.return_type, persist_dask=True)
    ...
    def _execute_sync_chunks(
        ...
        if max_concurrency is not None and max_concurrency > 1:
            with ThreadPoolExecutor(max_workers=max_concurrency) as executor:
                futures = [
                    executor.submit(execute_fn, **chunk_opts) for chunk_opts in chunk_opts_list
                ]
    ...
    def _combine_results(
        ...
        *,
        ...
        return (
            get_frame_strategy("dask").persist(combined)
    ...
    def _log_sync_fanout(
        ...
        *,
        ...
        if not options.get("diagnostics") or logger is None:
            return
        logger.info(
            "Gateway IN chunk fan-out sync: %d chunks, target=%s, chunk_size=%d, concurrency=%s",
            chunk_count,
            chunk_target,
            chunk_size,
            max_concurrency,
    ...
    def has_any_rows(frame: FrameResult) -> bool:
        try:
            return strategy_for_frame(frame).has_any_rows(frame)
        except Exception:
